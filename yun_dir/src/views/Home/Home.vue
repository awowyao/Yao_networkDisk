...
<template>
	<div ref="select_frame" style="width: 100%; height: 100%;" id="DirScrropHome" class="RightClick">
		<view style="margin-left: 0.5%;margin-bottom: 8px;display: block;">
			<el-button size="large" @click="updata">默认</el-button>
			<el-button size="large" @click="updata('#',1)"><el-icon><Sort /></el-icon></el-button>
		...
		methods: {
			...
			Get_file(path) {
				...
				if (FileW === 'mp4' || FileW === 'avi' || FileW === 'mkv') {
					if(!Route){
						location.href='/video?route=media' + Route + '/' + path + "&Sorted=" + this.sorted
			...
			handleBucketClick: function (data, node) {
				this.movePath = data.path + data.name
			},
			moveFileToDir() {
				var token = this.checkUser()
				axios({
